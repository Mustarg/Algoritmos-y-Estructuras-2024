...
class BinaryTree:
    ...
    class __Node:
        def __init__(self, value, left=None, right=None, other_value=None, other_other_value=None):
            self.value = value
            self.left = left
            self.right = right
            self.other_value = other_value
            self.other_other_value = other_other_value
            self.height = 0
    ...
    def insert_node(self, value, other_value=None, other_other_value=None):
        def __insert(root, value, other_value=None, other_other_value=None):
            if root is None:
                return BinaryTree.__Node(value, other_value=other_value, other_other_value=other_other_value)
            elif value < root.value:
                root.left = __insert(root.left, value, other_value, other_other_value)
            else:
                root.right = __insert(root.right, value, other_value, other_other_value)
            root = self.balancing(root)
            self.update_height(root)
            return root

        self.root = __insert(self.root, value, other_value, other_other_value)
    ...
    def contar_pokemons(self, tipo):
        def __contar_pokemons(root):
            counter = 0
            if root is not None:
                if tipo in root.value:
                    counter = 1
                counter += __contar_pokemons(root.left)
                counter += __contar_pokemons(root.right)
            return counter

        return __contar_pokemons(self.root)

    def inorden(self):
        def __inorden(root):
            if root is not None:
                __inorden(root.left)
                print(root.value, root.other_value, root.other_other_value)
    ...
    def inorden_pokemons(self):
        def __inorden_pokemons(root):
            if root is not None:
                __inorden_pokemons(root.left)
                if root.value is not None:
                    if "Agua" in root.value or "Planta" in root.value or  "Fuego" in root.value or "Eléctrico" in root.value:
                        print(root.other_value)
                __inorden_pokemons(root.right)

        if self.root is not None:
            __inorden_pokemons(self.root)
